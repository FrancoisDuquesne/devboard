...
export function useGithubTodos() {
  ...
  async function fetchTodos() {
    ...
    try {
      ...
      const fresh = await $fetch<DevBoardTodo[]>("/api/github/todos", { signal });
      ...
      if (previous.length > 0) {
        ...
        if (newTodos.length > 0) {
          toast.add({
            title: `${newTodos.length} new notification${newTodos.length > 1 ? "s" : ""}`,
  ...
  async function markAsDone(id: number) {
    ...
    try {
      await $fetch(`/api/github/todos/${id}`, { method: "POST" });
    } catch (e) {
  ...
  async function markAllAsDone() {
    ...
    try {
      await Promise.all(
        backup.map((t) => $fetch(`/api/github/todos/${t.id}`, { method: "POST" })),
      );
    } catch (e) {
      todos.value = backup;
      const message = e instanceof Error ? e.message : "Unknown error";
  ...
  async function fetchMentionMrs() {
    ...
    try {
      mentionMrs.value = await $fetch<DevBoardMR[]>("/api/github/mention-mrs", {
        signal,
      });
    } catch (e) {
      if (signal.aborted) return;
      console.warn("Failed to fetch mention PRs:", e);
    } finally {
